...
public class twosome_list extends AppCompatActivity {
    private ListView list;
    String[] name_list = new String[27];
    String[] price_list = new String[27];
    int [] draw = new int[27];

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_twosome_list);

        String tmp = makeDataString(R.raw.twosome_menu);
        makeClass(tmp);

        list = (ListView)findViewById(R.id.twosome);

        CustomList adapter = new CustomList(twosome_list.this);
